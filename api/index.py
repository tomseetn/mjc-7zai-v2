...
<div class="iti-wrap">
  <div class="iti-header">
    <img src="data:image/png;base64,__LOGO__" class="iti-logo" alt="MJC Logo">
    <div class="iti-company">
      <div class="iti-company-name">MJC Leisure Sdn Bhd</div>
      <div class="iti-company-sub">专业马来西亚入境旅游 · Since 2006</div>
    </div>
  </div>
  <div class="iti-title">[行程标题]</div>
  <div class="iti-meta">[人数] · [住宿等级] · [日期或"日期待定"]</div>
  <table class="iti-table">
    <thead><tr><th>天数</th><th>用餐</th><th>目的地</th><th>行程内容</th><th>住宿</th></tr></thead>
    <tbody>
      [每天一行 <tr><td>第X天</td><td class="meals">早✓/✗<br>午✓/✗<br>晚✓/✗</td><td>城市</td><td>活动详情</td><td>住宿城市</td></tr>]
    </tbody>
  </table>
  <div class="iti-sections">
    <div class="iti-section">
      <div class="iti-section-title">✅ 费用包含</div>
      <ul>[每项 <li>]</ul>
    </div>
    <div class="iti-section">
      <div class="iti-section-title">❌ 费用不含</div>
      <ul>[每项 <li>]</ul>
    </div>
    <div class="iti-section">
      <div class="iti-section-title">📝 备注</div>
      <p>[备注]</p>
    </div>
  </div>
...
用餐标记：含早→早✓ 不含→早✗，午晚同理。第一天通常全✗，最后一天早✓其余✗。
...
.iti-company-name{color:#fff;font-size:16px;font-weight:700}
.iti-company-sub{color:rgba(255,255,255,.8);font-size:11px;margin-top:3px}
...
.iti-table td.meals{font-size:12px;color:#444;white-space:nowrap;line-height:1.8}
.iti-sections{padding:14px 16px;display:flex;flex-direction:column;gap:10px}
.iti-section-title{font-weight:700;font-size:13px;margin-bottom:5px}
.iti-section ul{padding-left:18px;font-size:13px;color:#444}
.iti-section ul li{margin-bottom:2px}
.iti-section p{font-size:13px;color:#444}
...
async function send(){
  ...
  try{
    ...
    renderReply(reply);
  }catch(e){typing.remove();addMsg("连接错误，请检查网络","bot");}
  document.getElementById("sendBtn").disabled=false;
}
function renderReply(text){
  const m=text.match(/\[ITINERARY_START\]([\s\S]*?)\[ITINERARY_END\]/);
  if(m){
    const div=document.createElement("div");
    div.className="msg itinerary";
    div.innerHTML=m[1].trim();
...
function addMsg(text,type){
  ...
  if(type==="typing"||type==="bot"){
    div.innerHTML=text.replace(/
/g,"<br>").replace(//g,"<pre></pre>").replace(/\*\*(.+?)\*\*/g,"<b></b>");
  }else{
...
def get_history(phone):
    try:
        sb=get_supabase()
        res=(sb.table("conversations").select("role,content").eq("phone_number",phone).order("created_at",desc=True).limit(MAX_HISTORY).execute())
        rows=res.data or []
        rows.reverse()
...
def save_message(phone,role,content):
    try:
        get_supabase().table("conversations").insert({"phone_number":phone,"role":role,"content":content}).execute()
    except Exception as e:
...
def send_wati_message(phone,message):
    ...
    if not instance or not token:
        return
    try:
        r=httpx.post(f"{instance}/api/v1/sendSessionMessage/{phone}",json={"messageText":message},headers={"Authorization":f"Bearer {token}","Content-Type":"application/json"},timeout=15)
        r.raise_for_status()
    except Exception as e:
...
def ask_claude(messages,system):
    try:
        resp=get_anthropic().messages.create(model="claude-haiku-4-5-20251001",max_tokens=2048,system=system,messages=messages[-20:])
