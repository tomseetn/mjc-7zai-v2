...
def save_web_msg(session_id,role,content):
    try: get_supabase().table("website_conversations").insert({"session_id":session_id,"role":role,"content":content}).execute()
    except Exception as e: print(f"[DB web]{e}")
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path=="/api/chat":
            ...
            last_user=msgs[-1]["content"] if msgs and msgs[-1].get("role")=="user" else ""
            reply=ask_claude(msgs,SYSTEM_PROMPT)
            if last_user: save_web_msg(session_id,"user",last_user)
            save_web_msg(session_id,"assistant",reply)
            ...
            if not raw_json:
                # fallback: try to find JSON object directly in reply
                import re as _re
